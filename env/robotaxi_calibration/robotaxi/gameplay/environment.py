...
class Environment(object):
    ...
    def record_timestep_stats(self, result, agent_mode=0):
        ...
        if self.verbose >= 1 and self.stats_file is None:
            if self.participant is None: self.stats_file = open(f'robotaxi_calibration/csv/autocar_{self.field.size}_{timestamp}.csv', 'w')
            else: self.stats_file = open(f'robotaxi_calibration/csv/autocar_{self.field.size}_{self.participant}_{timestamp}.csv', 'w')
        ...
        if self.verbose >= 2 and self.debug_file is None:
            if self.participant is None: self.debug_file = open(f'robotaxi_calibration/log/autocar_{self.field.size}_{timestamp}.log', 'w')
            else: self.debug_file = open(f'robotaxi_calibration/log/autocar_{self.field.size}_{self.participant}_{timestamp}.log', 'w')            
    ...
    def record_timestep_stats_collaborator(self, result, agent_mode=0):
        ...
        if self.verbose >= 1 and self.stats_file_collaborator is None:
            self.stats_file_collaborator = open(f'robotaxi_calibration/csv/autocar_collaborator_{self.field.size}_{timestamp}.csv', 'w')
        ...
        if self.verbose >= 2 and self.debug_file_collaborator is None:
            self.debug_file_collaborator = open(f'robotaxi_calibration/log/autocar_collaborator_{self.field.size}_{timestamp}.log', 'w')
