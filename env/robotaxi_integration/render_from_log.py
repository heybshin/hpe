...
from robotaxi_integration.gui.pygame import QuitRequestedError
from robotaxi_integration.gameplay.entities import (Point, Field, CellType, SnakeDirection, ALL_SNAKE_DIRECTIONS, SNAKE_GROW, WALL_WARP)
...
class Colors:
    ...
    CELL_TYPE = {
        ...
        CellType.GOOD_EVENT: np.random.randint(20, 150, size=3) ,#(85, 242, 240),
        CellType.BAD_EVENT:  np.random.randint(80, 250, size=3),#(177, 242, 85),
...
def assign_selected_colors(proxy):
    ...
    Colors.CELL_TYPE[CellType.GOOD_EVENT] = permuted_color_assignment[0]
    Colors.CELL_TYPE[CellType.BAD_EVENT] = permuted_color_assignment[1]
...
class PyGameGUI:
    ...
    def __init__(self, field_size=8, noise_prob=0.0, num_noise_color=0, original=False, save_frames=False, proxy=0, stationary=False):
        ...
        self.frame_data_to_cell_type = {         
            '0': CellType.EMPTY,
            '1': CellType.GOOD_EVENT,
            '2': CellType.BAD_EVENT,
            '3': CellType.LAVA,
        ...
        self.object_type_to_reward = {
            CellType.GOOD_EVENT: 6,
            CellType.BAD_EVENT: -1,
            CellType.LAVA: -5,
        }
    ...
    def render_frame(self, frame_data, snake_direction, reward=None):
        ...
        if self.original:
            ...
            text_fields = ["+" + str(self.object_type_to_reward[CellType.GOOD_EVENT]), str(self.object_type_to_reward[CellType.BAD_EVENT]), str(self.object_type_to_reward[CellType.LAVA])]
            ct = 0
        ...
        for x in range(self.field_size):
            for y in range(self.field_size):
                ...
                if cell_type == CellType.EMPTY or cell_type == CellType.SNAKE_BODY:               
                    ...
                    if self.original:
                        if cell_type == CellType.GOOD_EVENT:
                            cell_coords = pygame.Rect(
                                x * self.CELL_SIZE + 5,
                                y * self.CELL_SIZE + 5,
                                self.CELL_SIZE,
                        ...
                        elif cell_type == CellType.BAD_EVENT:
                            cell_coords = pygame.Rect(
                                x * self.CELL_SIZE + 5,
                                y * self.CELL_SIZE + 20,
                                self.CELL_SIZE*2//3,
                            ...
                            if snake_direction == SnakeDirection.NORTH:
                                rotated_icon = self.north
                            elif snake_direction == SnakeDirection.WEST:
                                rotated_icon = self.west 
                            elif snake_direction == SnakeDirection.SOUTH:
    ...
    def render_scoreboard(self, score, time_remaining, frame_idx):
        ...
        if score[frame_idx] - score[frame_idx-1] == self.object_type_to_reward[CellType.GOOD_EVENT]:
        ...
        elif score[frame_idx] - score[frame_idx-1] == self.object_type_to_reward[CellType.BAD_EVENT]:
                    ...
                    if abs(x-x0) <= 1 and abs(y-y0) <= 1:                    
                        for interpolate_idx in range(1,self.intermediate_frames-1):
                            ...
                            if interpolate_idx < self.intermediate_frames / 3 * 2:
                                if reward == self.object_type_to_reward[CellType.GOOD_EVENT]: # good fruit
                                    cell_coords = pygame.Rect(
                                    ...
                                    self.screen.blit(self.good_fruit_icon, cell_coords)
                                elif reward == self.object_type_to_reward[CellType.BAD_EVENT]:
                                    cell_coords = pygame.Rect(
                                        self.curr_head[0] * self.CELL_SIZE + 5,
                                ...
                                if reward == self.object_type_to_reward[CellType.GOOD_EVENT]:
                                    cell_coords = pygame.Rect(
                ...
                if len(imgs) > 0 : #and frame_ct < 400
                    ...
                    for face_frame in face_frames:   
                        ...
                        if face_frame_ct < num_frames//2:
                            if reward == self.object_type_to_reward[CellType.GOOD_EVENT]: # good fruit
                                color = Colors.CELL_TYPE[CellType.GOOD_EVENT]
                                internal_square_coords = cell_coords.inflate((-self.internal_padding, -self.internal_padding))
                                pygame.draw.rect(self.screen, color, internal_square_coords)
                            elif reward == self.object_type_to_reward[CellType.BAD_EVENT]:
                                color = Colors.CELL_TYPE[CellType.BAD_EVENT]
                                internal_square_coords = cell_coords.inflate((-self.internal_padding, -self.internal_padding))
                                pygame.draw.rect(self.screen, color, internal_square_coords)
                            elif reward == self.object_type_to_reward[CellType.LAVA]:
...
def parse_command_line_args(args):
    ...
    parser = argparse.ArgumentParser(
        description='Replay recorded 20*20 snake game from log.'
    )
...
def main():
    ...
    max_step_limit = 0
    hit_wall_poses = []
    field_size = int(parsed_args.log_file.split('/')[-1].split('.')[0].split('_')[1])
