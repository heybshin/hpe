...
from robotaxi_integration.agent import AgentBase
from robotaxi_integration.gameplay.entities import CellType, Point, SnakeAction, SnakeDirection, ALL_SNAKE_ACTIONS, ALL_SNAKE_DIRECTIONS
    ...
    def __init__(self, grid_size=8, discount=0.95, reward_mapping=None, weights=None):
        ...
        if reward_mapping is None:
            self.reward_mapping = {
                ...
                CellType.GOOD_EVENT: 20,
                CellType.BAD_EVENT: -10,
                CellType.LAVA: -50,
                CellType.EMPTY: -1,
                CellType.WALL: -20,
            }
        ...
        for x in range(len(observation)):
            for y in range(len(observation[x])):
                if x != s_prime[0] or y != s_prime[1]:                 
                    if self.data_to_cell_type[observation[x][y]] == CellType.GOOD_EVENT or self.data_to_cell_type[observation[x][y]] == CellType.BAD_EVENT or self.data_to_cell_type[observation[x][y]] == CellType.LAVA:
    ...
    def train(self, env, num_episodes=1000, discount=0.95, exploration_range=(1.0, 0.1), exploration_phase_size=0.5, learning_range=(1.0, 0.1), learning_phase_size=0.5):
        ...
        for episode in range(num_episodes):
            ...
            while not game_over:
                ...
                for x in range(len(state_ob_next)):
                    for y in range(len(state_ob_next[x])):
                        if self.data_to_cell_type[state_ob_next[x][y]] == CellType.GOOD_EVENT or self.data_to_cell_type[state_ob_next[x][y]] == CellType.BAD_EVENT or self.data_to_cell_type[state_ob_next[x][y]] == CellType.LAVA:
