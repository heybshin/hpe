...
from robotaxi_integration.gameplay.environment import Environment
...
class ValueIterationAgent(AgentBase):
    ...
    def __init__(self, grid_size=8, discount=0.95, reward_mapping=None, env=None, level_file='./robotaxi_integration/levels/8x8-blank.json'):
        ...
        self.level_map_to_cell_type = {
            ...
            '#': CellType.WALL,
            'O': CellType.GOOD_EVENT,
            '.': CellType.EMPTY,
            'o': CellType.BAD_EVENT,
            '!': CellType.LAVA,
            'P': CellType.PIT,
            ...
            for y in range(len(observation[x])):
                ...
                if not self.env.collaboration:
                    if self.data_to_cell_type[observation[x][y]] == CellType.SNAKE_HEAD:
                        state_x = x
                        state_y = y
                        state_direction = self.env.vehicle.direction
