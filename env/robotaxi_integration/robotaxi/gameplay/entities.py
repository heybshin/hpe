...
class Snake(object):
    ...
    def __init__(self, start_coord, length=2, body_coord=None):
        ...
        self.need_car = False
        self.body = deque([Point(start_coord.x, start_coord.y)])
...
class Field(object):
    ...
    def create_level(self, init_cells=True):
        ...
        try:
            ...
            self._good_fruits = {
                ...
                if self[(x, y)] == CellType.GOOD_EVENT
            }
            self._bad_fruits = {
                Point(x, y)
                for y in range(self.size)
                for x in range(self.size)
                if self[(x, y)] == CellType.BAD_EVENT
            }
            self._lavas = {
                Point(x, y)
                for y in range(self.size)
    ...
    def update_snake_footprint(self, old_head, new_head, collaborator=None):
        """
        ...
        """
        self[old_head] = CellType.EMPTY

        if self[new_head] != CellType.WALL:
